...
class Tree
  attr_reader :children, :name, :weight
  ...
  def insert(tree)
    @children << tree unless @children.include? tree
  end
  ...
  def total_weight
    @total_weight ||= weight + children.map(&:total_weight).sum
  end

  def balanced
    @balanced ||= children.map(&:total_weight).uniq.length == 1
  end

  def child_weight_distribution
    weights = children.map(&:total_weight)
    odd_weight = weights.each_with_object(Hash.new(0)) { |weight, groups|
      groups[weight] += 1
    }.find { |_, count| count == 1 }.first
    unbalanced_child = children.find { |child| child.total_weight == odd_weight }
    balanced_child = children.find { |child| child != unbalanced_child }
    weight_difference = balanced_child.total_weight - unbalanced_child.total_weight
    unbalanced_child.weight + weight_difference
  end

  def unbalanced_subtree
    if children.all?(&:balanced)
      self
    else
      children.find { |child| !child.balanced }.unbalanced_subtree
    end
  end
end
...
programs = Hash[DATA.each_line.map do |line|
  po = ProgramOutput.new(line)
  [po.name, {output: po, tree: Tree.new(po.name, po.weight)}]
end]

trees = programs.reduce([]) do |trees, (name, program)|
  tree = program[:tree]
  program[:output].children.each do |child_name|
    tree.insert(programs[child_name][:tree])
  end
  trees << tree
end

# Prune
trees.flat_map(&:children).each do |child|
  trees.delete(child)
end

tree = trees.first

puts tree.unbalanced_subtree.child_weight_distribution
