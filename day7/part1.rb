class Tree
  attr_reader :children, :name, :value
  ...
  def contains?(value)
    root == value || children.map { |child| child.contains?(value) }
  end
...
programs = Hash[DATA.each_line.map do |line|
  ...
  [po.name, {output: po, tree: Tree.new(po.name, po.value)}]
end]
